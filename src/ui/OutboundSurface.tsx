...
export function OutboundSurface({
  ...
  return (
    <Surface title="outbound" meta={loading ? "loading" : `${targets.length} targets`} width={width} height={height}>
        ...
        visible.map((option, index) => {
          ...
          return (
            <ListRow
              ...
              selected={isSelected}
              text={option.target.display_name || option.target.target_id}
              textWidth={Math.max(8, contentWidth - 28)}
              suffix={truncate(option.target.channel, 10)}
              alignSuffix="end"
